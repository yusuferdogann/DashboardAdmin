      ...
      <div className="w-2/3 p-4 border-l">
        ...
        <div className="mt-4">
          ...
          <select
            ...
            onChange={() => setFormErrors((prev) => ({ ...prev, unit: false }))}
          >
        ...
        <div className="mt-4">
          ...
          <select
            ...
            onChange={(e) => {
              ...
              setSelectedMonth(""); // Yeni seçim türü değiştiğinde ayı sıfırla
              setSelectedPeriod(""); // Yeni seçim türü değiştiğinde dönemi sıfırla
        ...
        {selectionType && (
          <div className="mt-4">
            <label className="block">{selectionType === "month" ? "Ay" : "Dönem"}</label>
            {selectionType === "month" ? (
              <select
                className={`w-full p-2 border rounded ${formErrors.selection2 ? "border-red-500" : ""}`}
                value={selectedMonth}
                onChange={(e) => {
                  setSelectedMonth(e.target.value);
                  setFormErrors((prev) => ({ ...prev, selection2: false }));
                }}
              >
                <option value="">Seçiniz</option>
              ...
              >
                <option value="">Seçiniz</option>
                <option value="period1">Dönem 1</option>
                <option value="period2">Dönem 2</option>
                <option value="period3">Dönem 3</option>
              </select>
            )}
            {formErrors.selection2 && <small className="text-red-500">Bu alan boş olamaz.</small>}
          </div>
        )}

        {/* Yeni Select - Yangın Söndürme Tüpü dışında */}
        {showAdditionalSelect && (
          <div className="mt-4">
            <label className="block">Yeni Seçim</label>
            <select className="w-full p-2 border rounded">
              <option>Yeni Seçim 1</option>
              <option>Yeni Seçim 2</option>
            </select>
          </div>
        )}
