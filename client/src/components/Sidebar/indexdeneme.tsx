...
const Sidebar = () => {
  const [isFixed, setIsFixed] = useState(false); // Sidebar sabitleme durumu

  // Sidebar'ı açma/kapama kontrolü
  const toggleFixSidebar = () => setIsFixed(!isFixed);

  const userRole = "user"; // Örnek: userRole değişkeni burada sabit.

  return (
    <div
      className={`${
        isFixed ? "fixed z-9999 w-64" : "relative"
      } transition-all duration-300 ease-in-out bg-gray-800 text-white h-full ${
        isFixed ? "w-64" : "w-16" // Genişlik dinamik olarak ayarlanır
      } hover:w-64`}
    >
      {/* Sabitleme Checkbox */}
      <div className="absolute top-4 right-4">
        <input
          type="checkbox"
          checked={isFixed}
          onChange={toggleFixSidebar}
          className="cursor-pointer z-9999 mt-10"
        />
      </div>
